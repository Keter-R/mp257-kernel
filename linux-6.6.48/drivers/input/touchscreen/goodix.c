...
/* GT911 gesture/event registers from programming guide */
#define GT911_GESTURE_REG            0x814B   /* gesture type / event flag */
#define GT911_GESTURE_MODE_CMD       0x08     /* command 8 to 0x8046 then 0x8040 */
...
static int goodix_suspend(struct device *dev)
{
	...
	/*
	 * ---- Wakeup (gesture) path ----
	 * Keep IRQ as wake source, but DO NOT enable normal IRQ handling in suspend,
	 * otherwise the (documented / observed) spurious INT pulse when entering
	 * gesture mode can immediately wake the system.
	 */
	if (device_may_wakeup(dev)) {
		/*
		 * Disable normal interrupt handling first to avoid storm / immediate wake
		 * during mode switching and status clearing.
		 */
		disable_irq(client->irq);

		/* Enter GT911 gesture mode: write cmd 8 to 0x8046 then 0x8040 */
		error = goodix_i2c_write_u8(ts->client, 0x8046, GT911_GESTURE_MODE_CMD);
		if (error)
			goto out_reenable_irq;

		error = goodix_i2c_write_u8(ts->client, 0x8040, GT911_GESTURE_MODE_CMD);
		if (error)
			goto out_reenable_irq;
		...
		/*
		 * Clear possible pending “touch/coordinate” interrupt/status
		 * (your original logic).
		 */
		for (i = 0; i < 3; i++) {
		...
		/*
		 * IMPORTANT for GT911:
		 * In gesture mode, host should read/clear 0x814B (gesture event reg).
		 * If 0x814B is left non-zero, INT may stay asserted (level) or re-trigger.
		 */
		for (i = 0; i < 3; i++) {
			goodix_i2c_read(ts->client, GT911_GESTURE_REG, &dummy, 1);
			/* Clear gesture event */
			goodix_i2c_write_u8(ts->client, GT911_GESTURE_REG, 0x00);
			usleep_range(2000, 4000);
		}

		/*
		 * Arm as wake source WHILE normal IRQ handling stays disabled.
		 * Wakeup will still be generated by the interrupt controller.
		 */
...
out_reenable_irq:
	...
	/* -------- Non-wakeup path: keep your original low-power suspend logic -------- */
	...
	/* INT used as GPIO here, ensure IRQ already freed */
...
static int goodix_resume(struct device *dev)
{
	...
	/*
	 * If wakeup (gesture) path was used:
	 * 1) disable wake first
	 * 2) re-enable normal IRQ handling
	 *
	 * Do NOT free_irq here (unnecessary and can introduce GPIO/IRQ ownership issues).
	 */
	if (device_may_wakeup(dev)) {
	...
	if (ts->irq_pin_access_method == IRQ_PIN_ACCESS_NONE) {
		/* No IRQ pin access capability, follow original behavior */
		return 0;
	}

	/*
	 * Exit gesture mode reliably:
	 * Many platforms simply reset/sync INT then re-apply config if needed.
	 * Keep your original flow, but avoid changing INT direction unless you
	 * have actually freed the IRQ (non-wakeup path does that).
	 *
	 * If your hardware/driver requires INT sync, keep it:
	 */
	error = goodix_int_sync(ts);
	if (error)
		goto out_request_irq;

	/* Check config and restore (your original logic) */
...
out_request_irq:
	/*
	 * Ensure IRQ is requested in normal operation.
	 * - In wakeup path, IRQ was never freed, so goodix_request_irq()
	 *   should be able to handle “already requested” safely, OR you can
	 *   guard it in your implementation.
	 * - In non-wakeup path, IRQ was freed in suspend, so we must request it.
	 */
	error = goodix_request_irq(ts);
	return error;
}
