...
static irqreturn_t edt_ft5x06_ts_isr(int irq, void *dev_id)
{
	...
	for (i = 0; i < tsdata->max_support_points; i++) {
		...
		if (input_mt_report_slot_state(tsdata->input, MT_TOOL_FINGER,
			...
			// 		       x, y, true);	//alientek remove
			
			touchscreen_report_pos(tsdata->input, &tsdata->prop, y, x, true); //alientek add			
	}
...
static int edt_ft5x06_ts_probe(struct i2c_client *client)
{
	...
	/******************alientek add start*********************/
	struct device_node *lcd_np;
	unsigned int alientek_lcd_id = 0;
	lcd_np = of_find_node_by_path("/rgb_lcd_id");
	if (lcd_np == NULL) {
		dev_err(&client->dev, "rgb_lcd_id node not found!\n");
	}

	error = of_property_read_u32(lcd_np, "rgb_select_id", &alientek_lcd_id);
	if(error < 0) {
		dev_err(&client->dev, "rgb_select_id read failed!\n");
	}

	if (alientek_lcd_id == 1) {	//atk_rgb_lcd_7_800x480
		input_set_abs_params(input, ABS_MT_POSITION_X, 0, 800, 0, 0);
		input_set_abs_params(input, ABS_MT_POSITION_Y, 0, 480, 0, 0);
	} else if (alientek_lcd_id == 2) {	//atk_rgb_lcd_7_1024x600
